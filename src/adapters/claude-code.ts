 ...
 *   asserts every rule's baked patterns against the policy's match/notMatch
 *   examples, with the hook's own evaluator, and exits 1 on drift.
 */
export function generateHook(policy: CompiledPolicy): string {
  ...
  for (const rule of policy.rules) {
    const patterns = [
      ...(rule.pathRegex !== "" ? [rule.pathRegex] : []),
      ...rule.commandRegexes,
    ];
    if (patterns.length === 0) continue; // nothing baked to drift
    const alternation = patterns.join("|");
    for (const [kind, want, examples] of [
      ["match", "1", rule.match],
      ["notMatch", "0", rule.notMatch],
    ] as const) {
      // `?? []`: a hand-built CompiledRule (tests, older callers) may carry
      // no example arrays — that asserts nothing, exactly like a rule
      // authored without examples.
      (examples ?? []).forEach((ex, i) => {
        exampleCount++;
        selftestChecks.push(
          `  st_one ${shQuote(`rule ${rule.id} ${kind} example ${i + 1}`)} ${want} ${shQuote(ex)} ${shQuote(rule.notPathRegex)} ${shQuote(alternation)}`,
        );
      });
    }
  }
...
${selftestChecks.join("\n") || "  :"}
  if [ "$SELFTEST_FAIL" = "1" ]; then
    printf 'herkos hook selftest: FAILED — a baked rule drifted from its policy examples. Run herkos validate, then herkos init.\\n' >&2
    exit 1
  fi
  printf 'herkos hook selftest: %s rule(s), %s example(s) verified, path regex %s, stamp %s\\n' "${policy.rules.length}" "${exampleCount}" "\${PATH_RE:+set}" "$STAMP"
  exit 0
fi
