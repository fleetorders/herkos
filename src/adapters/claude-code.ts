...
export function generateHook(policy: CompiledPolicy): string {
  ...
  const pathCheck = (
    rule: { id: string; description: string; pathRegex: string },
    subject: "CMD" | "FP",
    kind: "Bash command" | "file path",
  ): string =>
    rule.pathRegex === ""
      ? ""
      : `    enforce ${shQuote(rule.id)} ${shQuote(rule.description)} ${shQuote(kind)} "$${subject}" ${shQuote(rule.pathRegex)}`;
  const cmdCheck = (
    rule: { id: string; description: string },
    re: string,
  ): string =>
    `    enforce ${shQuote(rule.id)} ${shQuote(rule.description)} ${shQuote("Bash command")} "$CMD" ${shQuote(re)}`;

  const bashChecks = policy.rules
    .flatMap((rule) => [
      pathCheck(rule, "CMD", "Bash command"),
      ...rule.commandRegexes.map((re) => cmdCheck(rule, re)),
    ])
    .filter((line) => line !== "")
    .join("\n");
  const fileChecks = policy.rules
    .map((rule) => pathCheck(rule, "FP", "file path"))
    .filter((line) => line !== "")
    .join("\n");
...
export const claudeCodeAdapter: HarnessAdapter = {
  ...
  wire(policy: CompiledPolicy): WireResult {
    ...
    return {
      changed,
      detail: `hook generated (${policy.ruleCount} rules, stamp ${stampOf(policy)}) and registered on PreToolUse for Bash + file tools; a session-start line reports enforcement each session. Takes effect for sessions started from now on.`,
    };
  },
  ...
  verify(): VerifyResult {
    ...
    const notes: string[] = [
      "hook present and registered on PreToolUse (Bash + file tools)",
    ];
    if (installed === null) {
      notes.push(
